...
const getFullAddress = (street, state, postalCode) =>
...
const expectedObjectMap = {
  fullname: "Fajar BC",
};
const expectedObjectMap2 = {
  name: "Fajar",
};
...
describe("normalize(object, map, preserve)", () => {
  ...
  test("It should return a normalized obj that doesn't preserve the other object properties", () => {
    expect(normalize(obj, map)).toEqual(expectedObjWithoutOtherProperties);
  });
  // Run n.normalize(obj, map, true)
  test("It should return a normalized obj that preserves the other object properties", () => {
    expect(normalize(obj, map, true)).toEqual(expectedObjWithOtherProperties);
  });
  test("Normalize object of Map with transform function", () => {
    expect(
      normalize(
        originObjectMap,
        [["fullname", "name", transformToFullname]],
        true
      )
    ).toEqual(expectedObjectMap);
  });
  test("Normalize object of Map without transform function", () => {
    expect(normalize(originObjectMap, [["name", "name.first"]])).toEqual(
      expectedObjectMap2
    );
  });
});
